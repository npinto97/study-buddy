...
- CRITICAL: Your entire output must be ONLY valid Python code. Do not add markdown fences, explanations, or any text before or after the code.
Code:"""

        response = self.llm.chat.completions.create(
            model="meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo",
            messages=[
                {"role": "system", "content": "You are a Python data visualization expert. You only output valid, executable Python code."},
    ...
    def _clean_code(self, code: str) -> str:
        """
        More robustly cleans the LLM's code output by removing markdown
        and truncating any text after the last essential code statement.
        """
        ...
        # Step 2: Find the last occurrence of a plotting command and truncate after it.
        last_code_keyword = 'plt.close()'
        last_keyword_pos = code.rfind(last_code_keyword)

        if last_keyword_pos != -1:
            end_of_line_pos = code.find('\n', last_keyword_pos)
            if end_of_line_pos != -1:
                return code[:end_of_line_pos + 1].strip()
            else:
                return code.strip()
    ...
    def _execute_and_save(self, code: str) -> List[str]:
        ...
        execution = self.sandbox.run_code(code)
        print("Cleaned code sent to sandbox:", code)
        
        if execution.error:
            error_details = f"Code execution failed: {execution.error.name} - {execution.error.value}\nTraceback:\n{execution.error.traceback}"
        ...
        if not saved_paths:
            print("Execution stdout:", execution.logs.stdout)
            print("Execution stderr:", execution.logs.stderr)
            raise RuntimeError("No visualizations were generated by the code.")
...
def create_data_analysis_tools() -> List[Tool]:
    ...
    global _code_interpreter_instance, _data_visualizer_instance
    tools = []

    # Code interpreter 
    try:
        if _code_interpreter_instance is None:
            _code_interpreter_instance = CodeInterpreter()
        
        def execute_code(code: str) -> str:
            """Safely execute code and return formatted results."""
            try:
                result = _code_interpreter_instance.run_code(code)
    ...
    try:
        if _data_visualizer_instance is None:
            _data_visualizer_instance = DataVisualizer()
        ...
        def create_visualization(csv_path: str, query: str) -> str:
            ...
            try:
                ...
                result = _data_visualizer_instance.create_visualization(resolved_path, query)
...
def close_sandboxes():
    """Chiude tutte le sandbox E2B attive create dai tool."""
    global _code_interpreter_instance, _data_visualizer_instance
    print("\nAttempting to close active sandboxes...")
    if _code_interpreter_instance:
        try:
            _code_interpreter_instance.close()
            print("Code Interpreter sandbox closed.")
        except Exception as e:
            print(f"Error closing Code Interpreter sandbox: {e}")
    if _data_visualizer_instance:
        try:
            _data_visualizer_instance.close()
            print("Data Visualizer sandbox closed.")
        except Exception as e:
            print(f"Error closing Data Visualizer sandbox: {e}")
